# This Makefile is for the Module::Crypt extension to perl.
#
...
#     ABSTRACT => q[Encrypt your Perl code and compile it into XS]
#     AUTHOR => q[Alessandro Ranellucci <aar@cpan.org>]
#     NAME => q[Module::Crypt]
#     PREREQ_PM => { File::Copy=>q[0], ExtUtils::ParseXS=>q[0], File::Spec=>q[0], Carp=>q[0], File::Find=>q[0], File::Temp=>q[0], File::Path=>q[0], ExtUtils::Mkbootstrap=>q[0], ExtUtils::CBuilder=>q[0] }
#     VERSION_FROM => q[Crypt.pm]
...
XSPROTOARG = 
XSUBPPDEPS = /System/Library/Perl/5.8.6/ExtUtils/typemap $(XSUBPP)
XSUBPPARGS = -typemap /System/Library/Perl/5.8.6/ExtUtils/typemap
XSUBPP_EXTRA_ARGS = 
...
DISTNAME = Module-Crypt
DISTVNAME = Module-Crypt-0.05
...
# Module::Crypt might depend on some other libraries:
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW)\
	  Crypt.pm $(INST_MAN3DIR)/Module::Crypt.$(MAN3EXT) 
...
clean :: clean_subdirs
	-$(RM_RF) Crypt.c ./blib $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all $(INST_ARCHAUTODIR)/extralibs.ld perlmain.c tmon.out mon.out so_locations pm_to_blib *$(OBJ_EXT) *$(LIB_EXT) perl.exe perl perl$(EXE_EXT) $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def lib$(BASEEXT).def $(BASEEXT).exp $(BASEEXT).x core core.*perl.*.? *perl.core core.[0-9] core.[0-9][0-9] core.[0-9][0-9][0-9] core.[0-9][0-9][0-9][0-9] core.[0-9][0-9][0-9][0-9][0-9]
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_F) $(INST_STATIC)
	$(RM_F)  $(INST_LIB)/Module/Crypt.pm $(MAKEFILE_OLD) $(FIRST_MAKEFILE)
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         Module-Crypt' >> META.yml
	$(NOECHO) $(ECHO) 'version:      0.05' >> META.yml
	$(NOECHO) $(ECHO) 'version_from: Crypt.pm' >> META.yml
	$(NOECHO) $(ECHO) 'installdirs:  site' >> META.yml
	$(NOECHO) $(ECHO) 'requires:' >> META.yml
	$(NOECHO) $(ECHO) '    Carp:                          0' >> META.yml
	$(NOECHO) $(ECHO) '    ExtUtils::CBuilder:            0' >> META.yml
	$(NOECHO) $(ECHO) '    ExtUtils::Mkbootstrap:         0' >> META.yml
	$(NOECHO) $(ECHO) '    ExtUtils::ParseXS:             0' >> META.yml
	$(NOECHO) $(ECHO) '    File::Copy:                    0' >> META.yml
	$(NOECHO) $(ECHO) '    File::Find:                    0' >> META.yml
	$(NOECHO) $(ECHO) '    File::Path:                    0' >> META.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Encrypt your Perl code and compile it into XS</ABSTRACT>' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="ExtUtils-CBuilder" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="ExtUtils-Mkbootstrap" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="ExtUtils-ParseXS" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  Crypt.pm $(INST_LIB)/Module/Crypt.pm 
	$(NOECHO) $(TOUCH) $@
